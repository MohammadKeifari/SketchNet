...
def parse_graph(json_data: dict) -> Graph:
    ...
    for n in json_data.get("nodes", []):
        ...
        for p in n.get("outputPorts", []):
            ...
            # If role port without explicit activation, set default based on role
        ...
        # Param ports (not used in data flow, ignore)
        # We don't need to store them.

        graph.add_node(node)

    # Special default for TrainTestSplit outputs (if not already set)
    for node in graph.nodes.values():
        if node.type == "train-test":
            for port in node.outputs:
                # If activation_mode is still default "every_batch" and not explicitly set in JSON, override based on subType
                # Check if the original JSON had an explicit activationMode for this port
                # We'll scan original node dict to see if it was present
                orig_node = json_data["nodes"][
                    list(graph.nodes.values()).index(node)
                ]  # not ideal, better to store original
                # Simpler: we'll apply override if the port's activation_mode == "every_batch" and we haven't found an explicit activationMode in the JSON for that port.
                # Since we already set activation_mode from JSON (if present) or default "every_batch", we can check if the port's original data had the key.
                # We'll need to revisit. I'll add a boolean flag.
                pass

    # Instead, we'll set the defaults in the port creation with explicit check.
    # I'll adjust the port creation to mark if activation_mode was explicitly provided.

    # For simplicity, we can do post-processing:
    for node in graph.nodes.values():
        if node.type == "train-test":
            for port in node.outputs:
                # If the original JSON did NOT contain activationMode for that port, set it based on subType
                # We'll reconstruct original ports dicts from json_data for that node
                orig_node = None
                for orig in json_data["nodes"]:
                    if orig["id"] == node.id:
                        orig_node = orig
                        break
                if orig_node:
                    orig_ports = orig_node.get("outputPorts", [])
    ...
    for l in json_data.get("links", []):
        link = Link(
            ...
            id_to=l["to"],
            weight=(
                l.get("weight", 1.0) if l.get("weight") != 0 else 1.0
            ),  # convert 0 to 1
