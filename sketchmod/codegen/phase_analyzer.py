...
def _active_nodes(graph: Graph, phase: str) -> Set[str]:
    ...
    A node is active only when every connected port (input / output / param)
...
def analyze_phases(graph: Graph) -> Dict[str, Any]:
    """
    Returns node sets and topological orders for each phase,
...
def highlight_path(graph_data: dict, phase: str) -> dict:
    """
    Returns node and link IDs that should be highlighted for the given phase.
    Strict rule: a link is highlighted only if BOTH ports have the phase.
    """
